...
if [[ "$(uname)" == "Darwin" ]]; then
    ...
    echo "Warning: This script is optimized for Mac. Consider using run_finetune.sh instead."
fi
...
# Try to install Mac-specific bitsandbytes
...
# Run finetuning with the smaller model
echo "Starting LoRA finetuning with smaller model (better for Mac)..."
python src/finetune_lora_small.py

# Check if model was saved
if [ -d "./lora_finetuned_small_model" ]; then
    echo "Finetuning complete! Model saved to ./lora_finetuned_small_model"
